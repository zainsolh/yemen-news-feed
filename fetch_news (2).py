...
# =========================
# قائمة المواقع المستهدفة المحدثة
# =========================
SOURCES = [
...
# =========================
# استخراج الأخبار الذكي والفلترة
# =========================
    ...
    try:
        ...
        if is_rss:
            ...
            for item in soup.select("a"):
                ...
                # تصفية الروابط والتبويبات غير المفيدة لعدم استهلاك الكوتا
...
def publish_to_blogger(token, title, summary, image, source, link):
    ...
    # حماية من الحجب السلبي: إذا كان الخبر قادماً من موقع رياضي محجوب محلياً (مثل beIN)
    # نقوم بإلغاء تضمين رابط الصورة المباشر لحماية نطاق مدونتك من الحظر الفرعي
    ...
    if image:
        ...
        # إذا لم تكن هناك صورة أو ألغيناها لحماية الـ DNS، نضع أيقونة رياضية/إخبارية جذابة جداً ومنسقة
    ...
    # --- إدارة تصنيفات الأقسام بدقة ---
    post_labels = [source, "أخبار اليمن"]
    
    # تحويل وفرز المقالات الرياضية تلقائياً لقسم الرياضة بمدونتك عبر التسمية "رياضة"
...
# =========================
# الدالة الرئيسية للتشغيل الممنهج
# =========================
def main():
    ...
    for a in articles:
        ...
        if success:
            ...
            # فاصل زمني آمن لتجنب حظر الـ API 429
